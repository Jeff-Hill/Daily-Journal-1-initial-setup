  ...
  for (let i = 0; i < deleteBtn.length; i++) {
    deleteBtn[i].addEventListener("click", () => {
      ...
      deleteEntry(btnId)
        ...
        // Before you can call the function below to re-fresh the DOM with the database after the deleted entry
        // is removed from it you need to chain a .then promise on to give enough time for the entry to be removed
        // before the trying to run the function
        .then(data => {
        // Call the function defined in data.js that empties the DOM and refreshes it with the database
        // after the entry is removed from it
...
function validateInputCharacters(concepttxt, entrytxt) {
  ...
  if (
    concepttxt.match(allowedCharacters) &&
    entrytxt.match(allowedCharacters)
  ) {
    alert("Thank You");
    return true;
  } else {
